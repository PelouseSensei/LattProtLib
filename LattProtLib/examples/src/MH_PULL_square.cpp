...
#include "config_MH_PULL_square.hpp"
...
int main() {
    ...
    SquareMetropolisSampler_pull SMCS(SEED, 1./T);
    
    // Configuration on the moves and their weights
    SMCS.setWeightMovei(0, 1.);
    auto pullb_move = SMCS.getMovei<PullBackwardSquare>(0);
    pullb_move->setBackboneDist(1,1);
    pullb_move->setMonomerDist(1, 4);
    
    SMCS.setWeightMovei(1, 1.);
    auto pullf_move = SMCS.getMovei<PullForwardSquare>(1);
    pullf_move->setBackboneDist(1,1);
    pullf_move->setMonomerDist(1, 4);
    
    SMCS.setWeightMovei(2, 7./4);
    auto pulle_move = SMCS.getMovei<PullEndSquare>(2);
    ...
    SMCS.setWeightMovei(4, 0);
    auto rotation_move = SMCS.getMovei<RotationSquare>(4);
    rotation_move->setBackboneDist(1,1);
    rotation_move->setDistMono(0,2);
    
    SMCS.setWeightMovei(5, 0.);
    auto symmetry_move = SMCS.getMovei<SymmetrySquare>(5);
    ...
    std::string results_filename  = "results_PULL_square.txt";
