...
void TranslationCubic::checkMove(Configuration<CubicLattice>& config) {
    ...
    for (int i = 0; i < moveArgs.b_length; i++) {
        ...
        dE -= config.computeLocalEnergyAndContactsIgnoringBackbonei(
            old_pos, type,
            moveArgs.old_contacts[i], moveArgs.old_counts[i],
            moveArgs.backbone_id);

        // Compute local energy + contacts for new position
        dE += config.computeLocalEnergyAndContactsIgnoringBackbonei(
            new_pos, type,
            moveArgs.new_contacts[i], moveArgs.new_counts[i],
            moveArgs.backbone_id);
    }
...
bool TranslationCubic::apply(Configuration<CubicLattice>& config) {
...
void TranslationCubic::generateMoveArgs(Configuration<CubicLattice>& config) {
    ...
    for (int i = 0; i < moveArgs.b_length; ++i) {
        ...
        t[2] += moveArgs.translation_vec[2];
